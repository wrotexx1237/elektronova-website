...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  const { data: pendingLeaveCount } = useQuery<{ count: number }>({
  ...
  const navItems = [
    ...
    { label: "Punëtorët", href: "/workers", icon: Users },
            ...
            {navItems.map((item) => (
              item.href === "/new" ? (
                ...
                >
                  <item.icon className="h-4 w-4" />
                  {item.label}
                  {item.label === "Punëtorët" && pendingLeaveCount && pendingLeaveCount.count > 0 && (
            ...
            {navItems.map((item) => (
              item.href === "/new" ? (
                ...
                >
                  <item.icon className="h-5 w-5" />
                  {item.label}
                  {item.label === "Punëtorët" && (pendingLeaveCount?.count || 0) > 0 && (
                     <Badge variant="destructive" className="ml-auto text-[10px] h-5 min-w-5 flex items-center justify-center rounded-full px-1">
...
function NotificationBell() {
  ...
  const typeLabels: Record<string, string> = {
    ...
    worker_request: "Kërkesë Punëtori",
    safety_submission: "Siguria (PMP)",
  };
  ...
  const typeColors: Record<string, string> = {
    ...
    worker_request: "text-pink-500",
    safety_submission: "text-emerald-500",
  };
  ...
  return (
    <Popover open={open} onOpenChange={(o) => {
      ...
      <PopoverContent className="w-80 p-0 z-[100]" align="end" sideOffset={8}>
        ...
        <div className="p-2 border-t text-center bg-popover rounded-b-md">
          <Link href="/notifications">
            <Button variant="ghost" size="sm" className="w-full text-xs font-bold text-primary hover:text-primary hover:bg-primary/5" onClick={() => setOpen(false)}>
