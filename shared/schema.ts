...
export const NOTIFICATION_TYPES = ["stale_offer", "upcoming_work", "low_stock", "price_change", "job_completed", "warranty_expiring", "payment_reminder", "worker_request", "safety_submission"] as const;
...
export const workers = pgTable("workers", {
  ...
  paymentMethod: text("payment_method").notNull().default("bank"), // "bank" or "cash"
  bankAccount: text("bank_account").notNull(),
  bankName: text("bank_name").notNull(),
  startDate: text("start_date").notNull(),
  contractType: text("contract_type").notNull().default("Caktuar"), // e.g. "Caktuar" vs "Pacaktuar"
  contractDuration: text("contract_duration"), // e.g. "1 Vit" or empty if indeterminate
  workingHours: integer("working_hours").notNull().default(40),
  workSchedule: text("work_schedule").notNull().default("Hëne - Premte, 08:00 - 16:00"),
  workplace: text("workplace").notNull().default("Zyrat qendrore / Terren"),
  probationPeriod: text("probation_period").notNull().default("1 Muaj"),
  workerSignature: text("worker_signature"), // base64 image
  employerSignature: text("employer_signature"), // base64 image
  portalToken: text("portal_token"), // Unique magic link token
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
});

export const insertWorkerSchema = createInsertSchema(workers).extend({
  email: z.string().email().nullable().optional().or(z.literal("")),
  salary: z.coerce.number().min(0),
  workingHours: z.coerce.number().min(0),
  paymentMethod: z.enum(["bank", "cash"]).optional().default("bank"),
...
export const leaveRequests = pgTable("leave_requests", {
  ...
  type: text("type").notNull().default("Vjetor"), // Vjetor, Mjekësor, Tjetër
  startDate: text("start_date").notNull(),
  endDate: text("end_date").notNull(),
  days: integer("days").notNull(),
  status: text("status").notNull().default("Në Pritje"), // Në Pritje, Aprovuar, Refuzuar
  reason: text("reason"),
  createdAt: timestamp("created_at").defaultNow(),
});
...
export const payslips = pgTable("payslips", {
  ...
  pensionContribution: real("pension_contribution").default(0),
  taxAmount: real("tax_amount").default(0),
  bonuses: real("bonuses").default(0),
  deductions: real("deductions").default(0),
  status: text("status").notNull().default("E Papaguar"), // E Papaguar, E Paguar
  notes: text("notes"),
...
export const workerHistory = pgTable("worker_history", {
  id: serial("id").primaryKey(),
  workerId: integer("worker_id").notNull(),
  changeType: text("change_type").notNull(), // Paga, Pozita, Departamenti, Kontrata
...
export const insertWorkerHistorySchema = createInsertSchema(workerHistory).omit({
  id: true,
  createdAt: true,
});
export type WorkerHistory = typeof workerHistory.$inferSelect;
